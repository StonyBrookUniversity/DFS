...
sub get_link_cost_per_sequence
{
	my $ClassID2PolicyChainRef = shift;
	my $ClassID2HostsRef = shift;
	my $SequencesPerClass = shift;
	...
	my $NodeIDtoNameRef = shift;
 	my $PathsRef = shift;
 	my $RoutesPerSequencePerClassRef = shift;

	my %LinkInClassSequence = ();
	my $class = "";
	foreach $class (keys %{$ClassID2PolicyChainRef})
	{
		my ($src,$dst) = split(/\;/,$ClassID2HostsRef->{$class});                             	
		#print "Class = $class $src $dst\n";
	        for (my $i =0 ; $i < $NumSequencesperClass->{$class}; $i++)
	        {
	        	my $routesequence = $RoutesPerSequencePerClassRef->{$class}->[$i]; 
	        	my @nodes = split(/\s+/,$routesequence);
	        	
			#print "RouteSequence = $routesequence\n";	
	        	my $classchain = "$class-$i";
	        	my $fullroute = "";
	        	for (my $j =0; $j < $#nodes; $j++)
	        	{
	        	 	my $nodeprev = $NodeIDtoNameRef->{$nodes[$j]};
	        		my $nodenext = $NodeIDtoNameRef->{$nodes[$j+1]};
				#print "Here2 $nodeprev $nodenext\n";
				my $canonicallink = get_canonical_linkname($nodeprev, $nodenext);
				$LinkInClassSequence{$canonicallink}->{$classchain}++;
	        	}
	        }
	}
	return (\%LinkInClassSequence);
}
