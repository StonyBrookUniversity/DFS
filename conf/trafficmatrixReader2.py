#!/usr/bin/python
import sys
fi = open("100_trafficmatrix", "r")
fj = open("100_topologylinks", "r")
fo = open("100_topology.dot", "wb")
sys.stdout=fo
print "graph test {"
...
######################################################################################################################
        if Flow[1] in RouterDict:
            if Flow[1] in SRouterDict: ##Case 1: If source-router exists in S (and R): Add new source
                ...
                RouterDict[Flow[1]][newSource].append(["ipsrc=10.%d.0.0/16"%(counter)])
                RouterDict[Flow[1]][newSource].append(["ipdst=10.%d.0.0/16"%(counter+5)])
                ...
                RouterDict[Flow[1]]['s1'].append(["ipsrc=10.%d.0.0/16"%(counter)])
                RouterDict[Flow[1]]['s1'].append(["ipdst=10.%d.0.0/16"%(counter+5)])
            ...
            SRouterDict[Flow[1]]['s1'].append(["ipsrc=10.%d.0.0/16"%(counter)])
            SRouterDict[Flow[1]]['s1'].append(["ipdst=10.%d.0.0/16"%(counter+5)])
...
######################################################################################################################
        if Flow[2] in RouterDict:
            if Flow[2] in DRouterDict: ## Case 1: If dest-router exists in D (and R): Add new ipdest
                ...
                RouterDict[Flow[2]]['ipdests'].append([" 10.%d.0.0/16 " % (counter + 5)])
                ##
            ...
            DRouterDict[Flow[2]]['ipdests'].append(["10.%d.0.0/16"%(counter+5)])
            ##
